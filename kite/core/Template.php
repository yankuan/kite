...
use core\exception\TemplateNotFoundException;
...
class Template
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $class         = false !== strpos($type, '\\') ? $type : '\\core\\template\\driver\\' . ucwords($type);
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\core\\template\\taglib\\' . ucwords($tagLib);
        }
            ...
            while ($matches[0]) {
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        if ('$core' == $first) {
                            // 所有以core.打头的以特殊变量对待 无需模板赋值就可以输出
                            $parseStr = $this->parsecoreVar($vars);
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $core. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parsecoreVar($vars)
    {
        ...
        if ($vars) {
            switch ($type) {
                ...
                case 'LANG':
                    ...
                    break;
                case 'CONFIG':
                    $parseStr = '\\core\\Config::get(\'' . $param . '\')';
                    break;
                default:
                    $parseStr = '\'\'';
                    break;
